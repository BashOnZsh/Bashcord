...
export const gitHash = process.env.BASHCORD_HASH || process.env.EQUICORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.BASHCORD_REMOTE || process.env.EQUICORD_REMOTE;
            if (!remote) {
