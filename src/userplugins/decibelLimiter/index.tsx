...
const settings = definePluginSettings({
    enabled: {
        ...
        description: "Activer DecibelLimiter",
        default: true
    },
    thresholdPercent: {
        type: OptionType.SLIDER,
        description: "Seuil critique (en %). Plus bas = plus sensible",
        default: 85,
    ...
    muteDurationMs: {
        type: OptionType.SLIDER,
        description: "Duree de coupure auto (ms)",
        default: 1200,
    ...
    cooldownMs: {
        type: OptionType.SLIDER,
        description: "Cooldown anti-spam (ms) par utilisateur",
        default: 2000,
    ...
    showToasts: {
        type: OptionType.BOOLEAN,
        description: "Afficher les notifications de protection",
        default: true
    },
    verboseLogs: {
        type: OptionType.BOOLEAN,
        description: "Afficher des logs detailles",
        default: false
    }
});
...
function scheduleRestore(connection: AnyConnection, userId: string, key: string) {
    ...
    const timeout = setTimeout(() => {
        ...
        try {
            ...
            log(`Volume restaure pour ${userId}: ${remembered}`);
        } catch (error) {
...
function applyHardMute(connection: AnyConnection, userId: string, normalizedLevel: number) {
    ...
    try {
        ...
        log(`Pic detecte (${percent}%) sur ${userId}. Volume coupe.`);
        toast(`DecibelLimiter: pic ${percent}% detecte, son coupe`, Toasts.Type.MESSAGE);
        scheduleRestore(connection, userId, key);
    } catch (error) {
        console.error("[DecibelLimiter] Erreur hard mute:", error);
    }
}
...
function attachStats(connection: AnyConnection) {
    ...
    log(`Listener stats attache sur connection ${connectionId}`);
}

function attachConnectionListener() {
    mediaEngine = MediaEngineStore?.getMediaEngine?.();
    if (!mediaEngine) {
        console.warn("[DecibelLimiter] MediaEngine indisponible");
        return;
    }
...
function cleanup() {
    ...
    for (const [connectionId, off] of statsUnsubscribers) {
        try {
            ...
            console.error(`[DecibelLimiter] Erreur detach ${connectionId}:`, error);
        }
    }
...
export default definePlugin({
    name: "DecibelLimiter",
    description: "Coupe automatiquement les pics sonores trop forts en appel vocal",
    authors: [{
        name: "Bash",
    ...
    start() {
        ...
        toast("DecibelLimiter actif", Toasts.Type.SUCCESS);
    },
