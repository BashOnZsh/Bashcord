...
async function runUpdateCheck() {
    ...
    try {
        ...
        showNotice(
            "A new version of Bashcord is available!",
            "View Update",
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Bashcord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Bashcord", err);
        }
    });
